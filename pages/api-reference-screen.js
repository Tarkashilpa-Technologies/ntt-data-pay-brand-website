...
const ApiReferenceScreen = () => {
  ...
  return (
    <div>
      {fullHeight && isReady ? (
        <div className=" overflow-auto w-100" style={{ maxHeight: fullHeight }}>
          <HeaderTwo />
          ...
          <div className="d-flex w-100 h-100" style={{ minHeight: 600 }}>
            <div
              ...
              className="bg-primary overflow-y-auto first-section-width d-lg-block d-none"
            >
              <div>
                {apisListData?.map((dropdown, index) => {
                  console.log(index)
            ...
            <div className="h-100 w-lg-50 w-100 middle-section-width">
              <div
                className="p-xl-5 pt-xl-2 pt-lg-2 middle-section-shadow overflow-y-scroll"
                style={{ maxHeight: fullHeight - 120 }}
              >
                <div className="text-start">
                  {" "}
                  <h1
                    className="text-start pb-3 title-font"
                    id={apiData?.attributes?.Title.replace(/\s+/g, "-")}
                  >
            ...
            <div
              className="d-lg-block d-none overflow-y-scroll"
              style={{ width: 400, height: fullHeight - 200 }}
            >
                    ...
                    <div key={index}>
                      ...
                      <div className="ps-4 ms-2">
                        {h1Data?.children?.map((h2Data, idx) => (
                          <div key={headersData?.length + idx}>
                            <a
                              href={`#${h2Data?.text?.replace(/\s+/g, "-")}`}
                              className={`d-flex fs-14 fw-semibold ${
                                selectedUrl == headersData?.length + idx
                                  ? "text-primary "
                                  : "text-Black"
                              }`}
                              onClick={() => {
                                setSelectedUrl(headersData?.length + idx);
                              }}
                            >
                              {selectedUrl == headersData?.length + idx && (
                                <span
                                  className="border-start-primary border-2 "
                                  style={{ marginLeft: -32, marginRight: 30 }}
                                ></span>
                              )}
                              {h2Data?.text}
                            </a>
