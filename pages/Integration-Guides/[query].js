...
import { tutorialGroupDataApi, useCaseDataApi } from "../../services/services";
...
const TutorialScreenPage = () => {
  ...
  const [
    pageFeedbackformSubmissionError,
    setPageFeedbackFormSubmissionMessage,
  ] = useState(null);
  const [
    pageFeedbackFormSubmissionStatus,
    setPageFeedbackFormSubmissionStatus,
  ] = useState("Not Initiated");
  ...
  const handleSubmit = async (event) => {
    ...
    await fetch("/api/integrationEmail", {
      ...
      if (res.status === 200) {
        setPageFeedbackFormSubmissionStatus("Success");
      } else {
        res?.json().then((body) => {
          setPageFeedbackFormSubmissionStatus("Failed");
          setPageFeedbackFormSubmissionMessage(
            body?.error ? body?.error : "Something went wrong!"
          );
        });
      }
    });
              ...
              <div className="d-block d-lg-none">
                <div className={`d-flex w-100 bg-primary`}>
                  <button
                    ...
                    style={{ minWidth: 140 }}
                    onClick={() => router.push("/Integration-Guides")}
                  >
                    {"<"} Back to home
                  </button>
                  ...
                  {tutorialData ? (
                    ...
                    >
                      ...
                      <div className="text-start">
                        ...
                        {/* related tutorials */}
                        <div>
                          {tutorialData?.Related_Tutorials?.data?.length >
                            0 && (
                            <div className="pb-3">
                        ...
                        {showHelpfulData &&
                          (isPageHelpful == null ? (
                            ...
                            <div>
                              ...
                              {pageFeedbackFormSubmissionStatus === "Failed" ? (
                                <div>{pageFeedbackformSubmissionError}</div>
                              ) : null}
                            </div>
                          ))}
                        {showHelpfulData && (
